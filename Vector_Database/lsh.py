...
class LSH:
    ...
    def __init__(
        ...
        n_perms: int = 128,
        weights = (0.5, 0.5),
        distance_function=distance.cosine_distance  # add this line
    ) -> None:
        ...
        self.store = dict() # make sure to store actual vectors and their corresponding keys here
        self.distance_function = distance_function  # store the distance function
    ...
    def index(self, vector: np.array, reference: object) -> None:
        ...
        self.store[reference] = (vector, [self.swap(hashed[s:e]) for s, e in self.ranges])  # store the vector and hashed values
        for hash_, table in zip(self.store[reference][1], self.tables):  # access the hashed values with index 1
    ...
    def query(self, vector: np.array) -> None:
        ...
        distances = [(candidate, self.store[candidate][1], self.distance_function(vector, self.store[candidate][0])) for candidate in candidates]
        return distances
